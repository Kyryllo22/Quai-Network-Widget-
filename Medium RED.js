...
function getBaseLayout() {
  return {
    ...
    burnContainerPaddingTop: 2, burnContainerPaddingBottom: 2,
    burnContainerPaddingLeft: 6, burnContainerPaddingRight: 6,
    fontSize: { price: 26, burnValue: 22 },
    topLogoSize: 26, bottomLogoSize: 22,
    priceColumnsGap: 15,
    burnLeftInnerGap: 4, burnRightInnerGap: 4,
    priceLogoGap: 4,
    arrowGap: 6,
  };
}
...
const CONFIG = (() => {
  ...
  return {
    ...
    spacing: {
      ...
      gaps: {
        priceColumns: fixed.priceColumnsGap,
        burnLeftInner: fixed.burnLeftInnerGap,
        burnRightInner: fixed.burnRightInnerGap,
        priceLogo: fixed.priceLogoGap,
        arrow: fixed.arrowGap
      }
    },
...
// 🔴 КРАСНАЯ РАМКА такой же толщины, как в большом красном виджете
function createStyledContainer(parent, bg = new Color("#1C1C1C", 0.3), pt, pb, pl, pr) {
  ...
  i.borderColor = Color.red();      // 🔴 красный цвет
  i.borderWidth = 3 * scale;        // такая же толщина, как в большом красном
...
function loadState() {
  ...
  const s = { price:null, mining:null, burned:null, timestamp:null, lastFlameTime:null, lastRocketTime:null, rocketBasePrice:null };
...
function saveState(state) {
  ...
  try {
    ...
    if (state.lastFlameTime != null) Keychain.set(KEY_LAST_FLAME_TIME, state.lastFlameTime.toString()); else Keychain.remove(KEY_LAST_FLAME_TIME);
...
function pruneOldCache() {
  try {
    const keys = [KEY_LAST_PRICE, KEY_LAST_MININGINFO, KEY_LAST_BURNED_BALANCE, KEY_DONOR_DATA, KEY_QI_RATE, KEY_DONOR_PRICES, KEY_SOAP_REVENUE, KEY_LAST_FLAME_TIME, KEY_LAST_ROCKET_TIME, KEY_ROCKET_BASE_PRICE];
    for (let key of keys) {
...
async function createWidget() {
  ...
  saveState({ price: marketPrice, burned: burned.value, timestamp: Date.now() });
  if (newPrice && st?.price) {
  ...
  let showRocket = (st.lastRocketTime && (Date.now() - st.lastRocketTime) < INDICATOR_WINDOW_MS && marketPrice >= st.rocketBasePrice);
  ...
  // ---- BOTTOM CONTAINER: Burned and Revenue ----
